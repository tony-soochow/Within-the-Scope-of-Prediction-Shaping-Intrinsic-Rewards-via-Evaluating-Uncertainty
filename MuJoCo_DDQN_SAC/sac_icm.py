...
class SAC(object):
    def __init__(self, num_inputs, action_space, args):
        ...
        self.predictor = ICMAgent(state_size=num_inputs, action_size=action_space.shape[0],
    ...
    def update_parameters(self, memory, batch_size, updates):
        ...
        tmp_next_state_batch = next_state_batch
        ...
        self.predictor.train(tmp_state_bathch, tmp_action_batch, tmp_reward_batch, tmp_next_state_batch)
...
class ActorPredict(nn.Module):
    def __init__(self, state_size=32, hidden_size=32, next_state_size = 32):
        super().__init__()
        self.fc1 = nn.Linear(state_size + next_state_size, hidden_size)
        self.fc2 = nn.Linear(hidden_size, hidden_size)
        self.fc3 = nn.Linear(hidden_size, 1)

    def forward(self, state, next_state):
        x = torch.cat([state, next_state], dim=1)  # 这里看看是1 还是 0
...
class ICMAgent(nn.Module):
    def __init__(self, state_size=4, action_size=10, encoded_state_size=32, ri_climp=10):
        ...
        self.actorpredict = ActorPredict(next_state_size=encoded_state_size)
        # 虽然没有直接训练编码器 但是间接训练了他的
        self.optimpre = torch.optim.Adam(self.prediction.parameters(), lr=1e-4)  # 先不去训练看看
        self.actoroptim = torch.optim.Adam(self.actorpredict.parameters(), lr=1e-4)
